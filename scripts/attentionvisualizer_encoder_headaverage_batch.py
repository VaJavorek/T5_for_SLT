...
avg_plot_folder = os.path.join(folder_path, "average_plots_auto_crop")
...
def find_global_bounding_box(matrices, threshold=NONZERO_THRESHOLD):
    ...
    If all values are below threshold (i.e., everything is effectively zero),
    returns None to indicate an all-zero region.
    """
    nonempty_rows_all = []
    nonempty_cols_all = []
...
for filename in json_files:
    ...
    # Assume the number of heads is the same in all layers (use first layer as reference)
    num_heads = len(encoder_attentions[0][batch_index])

    # Prepare a list to hold the averaged matrices for each head
    avg_matrices = []

    # -- For each head, find a global bounding box across all layers, then average --
    for head_index in range(num_heads):
        ...
        if bbox is None:
            # Everything is zero for this head across layers; create a 1x1 zero matrix
    ...
    # -- Now plot the averaged heads in a 3×4 grid --
    rows, cols = 3, 4
    ...
    # Add a main title
    fig.suptitle(
        f"File: {filename}\nAveraged Attention Across Layers (Auto-Cropped), Batch 1",
        fontsize=14
    )
    ...
    for head in range(num_heads):
        if head >= rows * cols:
            # If more than 12 heads, stop plotting or adjust grid
            break
    ...
    # Save the figure
    out_filename = f"{os.path.splitext(filename)[0]}_avg_heads_auto_crop.png"
    out_filepath = os.path.join(avg_plot_folder, out_filename)
    plt.savefig(out_filepath, dpi=150)
    plt.close(fig)

    print(f"Saved average auto-cropped plot: {out_filepath}")